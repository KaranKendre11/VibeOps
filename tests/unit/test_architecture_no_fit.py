...
def test_all_zones_zero_quota_returns_quota_error() -> None:
    ctx = _ctx()
    zones = [
        ZoneAvailability(zone="us-central1-a", region="us-central1", gpu_available=True, quota_total=0, quota_used=0),
    ]
...
def test_no_zones_found_returns_error() -> None:
    ctx = _ctx()

    with (
        patch("vibeops.agents.architecture.list_zones_with_accelerator", return_value=ZonesWithAcceleratorResult(gpu_type="nvidia-tesla-t4", zones=[])),
        ...
        patch("vibeops.agents.architecture.list_machine_types", return_value=MachineTypesResult(zone="us-central1-a", machine_types=[])),
        patch("vibeops.agents.architecture.list_existing_networks", return_value=NetworksResult(networks=[])),
    ):
        result = architecture_agent(
            GraphState(user_prompt="T4 VM", requirement_draft=_draft()),
...
def test_no_matching_machine_returns_error_stage_requirement() -> None:
    ctx = _ctx()
    zones = [
        ZoneAvailability(zone="us-central1-a", region="us-central1", gpu_available=True, quota_total=4, quota_used=0),
    ]
    # Machine doesn't meet memory floor (32 GB required, 16 GB available)
    machines = [MachineType(name="n1-standard-4", cpus=8, memory_gb=16.0, gpu_compatible=True)]
    ...
    assert result.error is not None
    assert result.stage == FlowStage.REQUIREMENT
